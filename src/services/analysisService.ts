...
export async function buildAnalysis(
  ...
  // The per-fixture bundle (Flashscore) keys teams by name, so re-key the
  // fixture's own teams the same way before computing stats/H2H — otherwise a
  // fixture seeded without real provider ids (e.g. opened from a saved bet or
  // history record) wouldn't match any of its own games ("dados insuficientes").
  const homeTeam = bundle ? { ...fixture.home, id: teamKey(fixture.home.name) } : fixture.home;
  const awayTeam = bundle ? { ...fixture.away, id: teamKey(fixture.away.name) } : fixture.away;
  ...
  let homeStats = computeTeamStats(homeTeam, homeMatches, { now });
  let awayStats = computeTeamStats(awayTeam, awayMatches, { now });
  ...
  const h2h = computeHeadToHead(homeTeam.id, awayTeam.id, h2hPool, 10, { now });
